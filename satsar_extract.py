...
Notes:
- Expects runway coordinates in WGS-84 (EPSG:4326).
...
def resolve_in_data(p: Path) -> Path:
    """
    If 'p' exists as given, return it.
    Otherwise, try DATA_DIR / p.
    """
    if p.exists():
        return p
    ...
    if candidate.exists():
        return candidate
    return p  # let caller handle the not-found error
...
def run_extract(image_filename: Path, data_filename: Path, output_dir: Path) -> list[Path]:
    ...
    airfield_col = (name_cols.get("airfield_name") or
                    name_cols.get("airfield") or
                    name_cols.get("airport") or None)
    airstrip_col = (name_cols.get("airstrip_name") or
                    name_cols.get("airstrip") or
                    name_cols.get("runway") or None)
...
def main():
    ...
    parser.add_argument("image_filename", help="GeoTIFF image (basename or path; looks in ./data by default)")
    parser.add_argument("data_filename", help="CSV/XLSX with runway corners (basename or path; looks in ./data)")
    parser.add_argument(
        "--outdir",
    ...
    # Resolve paths (prefer given path; otherwise look under ./data)
    image_path = resolve_in_data(Path(args.image_filename))
    data_path = resolve_in_data(Path(args.data_filename))

    if not image_path.exists():
        print(f"[ERROR] Image not found: {image_path} (also checked {DATA_DIR / args.image_filename})", file=sys.stderr)
        sys.exit(1)
    if not data_path.exists():
        print(f"[ERROR] Data file not found: {data_path} (also checked {DATA_DIR / args.data_filename})", file=sys.stderr)
        sys.exit(1)
